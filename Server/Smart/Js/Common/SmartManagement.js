...
InforCenter_Platform_SmartManagement_OnRowClick = function (pageEvent) {
    ...
    if (pagePara.IsTab) {
        pagePara.ObjectID = SelectRowData.EID;
        var objectTypes = pagePara.LObjTypes;
        if (objectTypes != undefined && (HoteamUI.Common.IsNullOrEmpty(objectTypes[0].PageLinksName) == false)) {
            //根据PageLinksName获取一组PageLink
            var data = InforCenter_Platform_GetTreeManagePageLinksByName(objectTypes[0].PageLinksName);
        }
        if (data != null) {
