(function (factory){
    ...
    function UploadPreview(options) {
        ...
        this.builder = {
            ...
            buildPreviewBox: function (options, toolBar,dataIndex) {/*创建图像预览框*/
                var htmlArr = [],
                    previewElement = "div",
                    previewClass = "";
                ...
                //21021 李金岳
                htmlArr.push('<' + previewElement + ' class="preview-box' + previewClass + '" data-index="' + dataIndex + '">');
    ...
    UploadPreview.prototype.render = function (WuFile, onlyDel) {
        ...
        if (panel.length > 0) {
            ...
            panel.children('span').on("click", function () {
                ...
                //21021 李金岳
                var index = panel.parent(".preview-box").attr("data-index");
                ...
                if ($this.hasClass('cancel')) {//删除按钮
                    ...
                    if (WuFile.getStatus() === "complete") {
                        if (that.options.onDelUploaded && $.isFunction(that.options.onDelUploaded)) {
                            ...
                            //21021 李金岳
                            that.options.onDelUploaded(id, index);
    ...
    */
    ...
    UploadPreview.prototype.loadImage = function (src, dataIndex) {
        ...
        if (this.options.previewInfo.showToolBtn) {
            //创建工具按钮
            toolBar = builder.buildTool(true, {});
        }
        //创建图像预览框
        previewBox = builder.buildPreviewBox({
            previewElement: this.options.previewInfo.previewElement,
        ...
        if (panel.length > 0) {
            ...
            panel.children('span').on("click", function () {
                ...
                //21021 李金岳
                if (that.options.onDelUploaded && $.isFunction(that.options.onDelUploaded)) {
                    that.options.onDelUploaded(null,index);
                }
            });
        }
        ...
        $(img).one("load", function () {
            ...
            if (width > 0 && height > 0) {
                ...
                wh = UploadPreview.calculateWh(imgWrapWh.width, imgWrapWh.height, $(this).width(), $(this).height());
            }
            ...
            if (that.options.previewInfo.viewImgHorizontal) {
                $(this).css({
                    ...
                    "left": "50%",
                    "top": "50%",
                    "margin-left": -wh.width / 2,
                    "margin-top": -wh.height / 2
                });
            }
        });
