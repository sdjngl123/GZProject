...
hoteamMiniCAD = {
    ...
    create: function (options) {
        ...
        //控件初始化
        var activeXObj = "";
        activeXObj = activeXObj + '<object id="' + o.id + '" CLSID:D8A5D60F-AAFE-4F26-9B30-C89E6A27857C"    ';
    ...
    changeMode: function (options) {
        //参数格式{id:null,mode:null}
        var o = $.extend({}, hoteamMiniCAD.options, options);
        var activeXObj = $("#" + o.id);
        if (activeXObj.length > 0) {
            activeXObj = activeXObj[0];
            if (o.mode == "edit") {
