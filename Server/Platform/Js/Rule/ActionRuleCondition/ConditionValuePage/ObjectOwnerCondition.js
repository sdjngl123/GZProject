//对象授权结果加载页面
InforCenter_Platform_ObjectOwnerCondition_LoadPage = function (pageEvent) {
    ...
    if (HoteamUI.Common.IsNullOrEmpty(para.ConditionValue) == false) {
        if (HoteamUI.Common.IsNullOrEmpty(para.ConditionValue.Parameter1) == false) {
            var objectPermissionValue = page.GetControl("ObjectOwnerValueText");
...
//获取页面保存数据
InforCenter_Platform_ObjectOwnerCondition_OnGetDataFromPage = function (pageEvent) {
    InforCenter_Platform_Object_Data = null;
    var bCheck = true;
    var data = {};
    {
        var c = pageEvent.o.GetControl('ObjectOwnerValueText');
...
InforCenter_Platform_ObjectOwnerCondition_OwnerOnClick = function (ctrlEvent) {
    ...
    HoteamUI.UIManager.Popup("TreeCommonQuery", { Value: userID, ItemName: "GroupToRoleToUser", LoadAndSelectType: "SingleLevel_SingleSelect", AllQuery: true }, callback, "", "320*500");
}