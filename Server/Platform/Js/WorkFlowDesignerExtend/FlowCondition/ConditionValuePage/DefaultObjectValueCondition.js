//对象授权结果加载页面
InforCenter_Platform_DefaultObjectValueCondition_LoadPage = function (pageEvent) {
    ...
    if (HoteamUI.Common.IsNullOrEmpty(objectPermissionValue) == false) {
        if (HoteamUI.Common.IsNullOrEmpty(para.Value) == false) {
            ...
            if (HoteamUI.Common.IsNullOrEmpty(conditionValue.Data) == false) {
                objectPermissionValue.SetText(conditionValue.Data.Value);
            }
        }
    }
}
...
//获取页面保存数据
InforCenter_Platform_DefaultObjectValueCondition_OnGetDataFromPage = function (pageEvent) {
