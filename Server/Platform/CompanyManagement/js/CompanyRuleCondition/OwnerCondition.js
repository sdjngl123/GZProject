InforCenter_Platform_OwnerCondition_LoadPage = function (pageEvent) {
    ...
    if (HoteamUI.Common.IsNullOrEmpty(para.ConditionValue) == false
        ...
        if (user) {
            page.GetControl("OwnerValueCtrl").SetText(user.ENAME_DisplayValue);
            page.GetControl("OwnerValueCtrl").SetValue(eid);
        }
    }
}
InforCenter_Platform_OwnerCondition_OnGetDataFromPage = function (pageEvent) {
    InforCenter_Platform_Object_Data = null;
    var bCheck = true;
    var data = {};
    {
        var c = pageEvent.o.GetControl('OwnerValueCtrl');
        if (c.Check() == false) {
            bCheck = false;
            return;
        }
...
InforCenter_Platform_OwnerCondition_OwnerOnClick = function (ctrlEvent) {
