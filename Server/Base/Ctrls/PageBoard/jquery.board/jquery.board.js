(function ($, window, undefined) {
    ...
    Board.prototype.renderCanvas = function () {
        var self = this,
            hash = this.hash,
            renderCanvas = this.opt.render.canvas,
            index = 1;

        this.elem.find('.hq-board-canvas-content').each(function () {
            var $elem = $(this),
                elem = this;

            setTimeout(function () {
                renderCanvas(elem, $.extend({}, hash[$elem.attr('data-id')]));
            }, this.interval * index++);
        });
    }
    ...
    Board.prototype.renderResize = function () {
        var self = this,
            hash = this.hash,
            renderResize = this.opt.render.resize;

        this.elem.find('.hq-board-canvas-content').each(function () {
            var $elem = $(this);
            renderResize(this, $.extend({}, hash[$elem.attr('data-id')]));
        });
    }
